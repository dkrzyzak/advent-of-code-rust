...
use regex::Regex;
use super::{Circuit, FinalCircle, Module, ModuleType};
...
pub fn extract_input(lines: &Vec<String>) -> (Circuit, FinalCircle)  {
    let mut circuit = Circuit::new();
    let mut final_circle = FinalCircle::new();
    ...
    for (name, module) in circuit.clone().iter_mut() {
        for output in module.outputs.iter() {
            // PART 2
            if *output == String::from("df") {
                final_circle.insert(name.clone());
            }
